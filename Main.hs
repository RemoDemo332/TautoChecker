...
module Main where
    ...
    -- | Parse an RPN expression in the form PQRAKPQKC into a Proposition.
    parseRPN :: String -> Proposition
    parseRPN = fst . foldl' go (N (Const False), [])
        where
            go (p, ps) v =
                case v of
                    'A' -> (A p (head ps), tail ps)
                    'K' -> (K p (head ps), tail ps)
                    'C' -> (C p (head ps), tail ps)
                    'D' -> (D p (head ps), tail ps)
                    'E' -> (E p (head ps), tail ps)
                    'J' -> (J p (head ps), tail ps)
                    'N' -> (N p, ps)
                    _   -> (Var [v], p:ps)
            

            -- | Convert a Proposition to prefix notation.
            toPrefix :: Proposition -> String
            toPrefix (Const b) = show b
            toPrefix (Var v) = v
            toPrefix (A p q) = "A " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (C p q) = "C " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (D p q) = "D " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (E p q) = "E " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (J p q) = "J " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (K p q) = "K " ++ toPrefix p ++ " " ++ toPrefix q
            toPrefix (N p) = "N " ++ toPrefix p
